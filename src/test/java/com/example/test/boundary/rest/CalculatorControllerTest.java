...
class CalculatorControllerTest {
  ...
  private static final String DIV_ENDPOINT = "/div";
  private static final String VALUE = "$.value";

  @Autowired private MockMvc mockMvc;

  @MockitoBean private CalculatorService calculatorService;

  @ParameterizedTest
  @DisplayName("POST add endpoint should return OK status with correct value")
  @MethodSource("sumArguments")
  void addShouldReturnCorrectValue(double val1, double val2, double expected, String requestBody)
      throws Exception {
    when(calculatorService.add(val1, val2)).thenReturn(expected);

    mockMvc
        .perform(post(ADD_ENDPOINT).contentType(MediaType.APPLICATION_JSON).content(requestBody))
        .andExpect(status().isOk())
        .andExpect(jsonPath(VALUE).value(expected));
  }

  @ParameterizedTest
  @DisplayName("POST add endpoint should return BAD REQUEST when field is missing")
  @MethodSource("sumArgumentsWithMissingField")
  void addShouldReturnBadRequestWhenFieldIsMissing(String requestBody, String expectedMessage)
      throws Exception {
    mockMvc
        .perform(post(ADD_ENDPOINT).contentType(MediaType.APPLICATION_JSON).content(requestBody))
  ...
  @ParameterizedTest
  @DisplayName("POST add endpoint should return BAD REQUEST when passed field has invalid type")
  @MethodSource("sumArgumentsWithInvalidType")
  void addShouldReturnBadRequestWhenFieldHasInvalidType(String requestBody, String expectedMessage)
      throws Exception {
    mockMvc
        .perform(post(ADD_ENDPOINT).contentType(MediaType.APPLICATION_JSON).content(requestBody))
        .andExpect(status().isBadRequest())
  ...
  void addShouldReturnBadRequestWhenInvalidJsonWasPassed(String requestBody, String expectedMessage)
      throws Exception {
    mockMvc
        .perform(post(ADD_ENDPOINT).contentType(MediaType.APPLICATION_JSON).content(requestBody))
        .andExpect(status().isBadRequest())
  ...
  void divShouldReturnBadRequestWhenParamIsMissing(
      MultiValueMap<String, String> params, String expectedMessage) throws Exception {
    mockMvc
        .perform(get(DIV_ENDPOINT).params(params))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.timestamp").exists())
        .andExpect(jsonPath("$.status").value(400))
        .andExpect(jsonPath("$.errorName").value(HttpStatus.BAD_REQUEST.getReasonPhrase()))
  ...
  void divShouldReturnBadRequestWhenBothParamsHaveInvalidType(
      String val1, String val2, List<String> expectedMessages) throws Exception {
    mockMvc
        .perform(get(DIV_ENDPOINT).param("val1", val1).param("val2", val2))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.timestamp").exists())
        .andExpect(jsonPath("$.status").value(400))
        .andExpect(jsonPath("$.errorName").value(HttpStatus.BAD_REQUEST.getReasonPhrase()))
  ...
  @ParameterizedTest
  @DisplayName(
      "GET div endpoint should return BAD REQUEST when one of the params have invalid type")
  @MethodSource("divideArgumentsWithInvalidType")
  void divShouldReturnBadRequestWhenOneParamHaveInvalidType(
      String val1, String val2, String expectedMessage) throws Exception {
    mockMvc
        .perform(get(DIV_ENDPOINT).param("val1", val1).param("val2", val2))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.timestamp").exists())
  ...
  void divShouldReturnBadRequestWhenDivisorIsZero() throws Exception {
    ...
    mockMvc
        .perform(get(DIV_ENDPOINT).param("val1", "5").param("val2", "0"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.timestamp").exists())
        .andExpect(jsonPath("$.status").value(400))
        .andExpect(jsonPath("$.errorName").value(HttpStatus.BAD_REQUEST.getReasonPhrase()))
  ...
  private static Stream<Arguments> sumArgumentsWithMissingField() {
    return Stream.of(
        Arguments.arguments(
            """
                        {
  ...
  private static Stream<Arguments> divideArgumentsWithMissingParam() {
    return Stream.of(
        Arguments.arguments(
            MultiValueMap.fromSingleValue(Map.ofEntries(entry("val1", "2"))),
        ...
        Arguments.arguments(
            MultiValueMap.fromSingleValue(Map.ofEntries(entry("val2", "2"))),
